...
This notebook represents a departure from the usual order in which one
learns basic concepts in NLP. And, perhaps, in this LLM era, there is no
usual progression anymore!

Pedagogically, **[Jurafsky and Martin (3rd.
ed.)](https://web.stanford.edu/~jurafsky/slp3/)** sets a standard by
which many follow. Yet, it’s an incredibly lengthy and dense book
oriented largely towards graduate students.

The challenge is learning NLP using an incremental and human
language-centered approach to NLP, while also committing to libraries
that students will be expected to know as data scientists in the work
environment.

**[SpaCy v.3](https://spacy.io)** provides a staggeringly rich ecosystem
for NLP development suitable both for data science experimentation and
enterprise deployment. It is a defacto industry standard that integrates
well with other common tools used by data scientists.

This notebook is a whirlwind tour of spaCy. You will gain familiarity
with spaCy container objects – notably, **Doc** (sequence of token
objects) and **Token** objects. Tokens themselves contain text
annotations as processed by components in the spaCy pipeline. *Don’t
worry if you don’t understand everything that’s going on. The hope is
that by the end of the class, you will!*

This notebook is also designed to tackle linguistic concepts as we
encounter them, while starting from the most natural place to learn
linguistic processing – within the spaCy pipeline.

Finally, we’re going to tackle learning traditional NLP methods
alongside LLMs. Traditional methods include predictive AI methods that
well complement generative AI techniques. We’re going to think about
when and how to use both… and sometimes together.
...
**Your goal** is to read this notebook and experiment as you like. I
...
with AI to learn and expose capabilities in spaCy. Wherever you see
this, 🤖Reply🤖, it’s Claude inside of Solveit answering questions that
I’ve posed. (In some cases, I’ve post-edited these responses.) Having
such assistance makes it possible to learn and do more than one might,
otherwise.
...
Below we’re loading some utility libs that will be used for LLM
prompting later in the notebook. Your model is currently set to claude,
though you can change it to OpenAI by `LLM_MODELS[0]`

It is easy to extend the helper to accommodate others and send a [pull
...
<figure>
<img src="attachment:7d560d89-c8fe-4a24-b742-16a8368e7184.png"
...
<figure>
<img src="attachment:b62affb2-2519-4fe4-82cf-f40dcffa03e3.png"
...
<figure>
<img src="attachment:acf6b95c-51de-43b2-bdee-7a74d9bdd17a.png"
...
if len(llm_results) < len(test_utterances):
    print("⚠️ Some LLM sentiment analyses failed; comparison table may be incomplete.")
```
